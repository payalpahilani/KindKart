...
export default function SignUpScreen({ navigation }) {
  ...
  useEffect(() => {
    if (response?.type === 'success') {
      ...
      signInWithCredential(auth, credential)
        .then(async (userCred) => {
          // Save user info to Firestore on Google login
          ...
          navigation.replace('ChatList'); // Navigate to chat list after login/signup
        })
  ...
  const handleSignUp = async () => {
    ...
    try {
      ...
      navigation.replace('ChatList'); // Navigate to chat list screen after signup
    } catch (err) {
