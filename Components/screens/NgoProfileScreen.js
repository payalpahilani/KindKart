...
export default function NgongoProfileScreen() {
  ...
  // Fetch NGO details and then stats
  ...
  // New: sum actual received donations
  const fetchCampaignStats = async (uid) => {
    try {
      ...
      if (count === 0) {
        setCampaignStats({ count: 0, total: 0 });
        return;
      }

      // 2) Firestore "in" queries are limited to 10 IDs at a time
      const chunkSize = 10;
      let totalReceived = 0;
      ...
      for (let i = 0; i < campaignIds.length; i += chunkSize) {
        ...
        donSnap.forEach((d) => {
          ...
          if (!isNaN(amt)) totalReceived += amt;
        });
      }

      setCampaignStats({ count, total: totalReceived });
    } catch (err) {
  ...
  const LangChip = ({ code, label, flag }) => {
    ...
    return (
      ...
      >
        <Text style={{ color: active ? '#fff' : text, fontWeight: '600' }}>
          {flag} {label}
        </Text>
      </Pressable>
    );
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: bg }]}>
      ...
      >
        ...
        <View style={styles.header}>
          ...
          <Text style={[styles.name, { color: text }]}>{ngo?.ngoName}</Text>
        ...
        >
          ...
          <InfoRow
            ...
            value={`$${campaignStats.total.toFixed(2)}`}
            textColor={text}
            mutedColor={muted}
          />
        </View>
        ...
        >
          ...
          <Text style={[styles.optionText, { color: text }]}>FAQ</Text>
        </TouchableOpacity>
        <TouchableOpacity
          style={styles.option}
