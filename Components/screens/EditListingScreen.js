...
export default function EditListingScreen({ route, navigation }) {
  ...
  const [ngoLabel, setNgoLabel] = useState(ad.ngoName || "");
  const [cause, setCause] = useState(ad.cause || "");
  const [causeLabel, setCauseLabel] = useState(ad.causeName || "");
  ...
  const [pickupCoords, setPickupCoords] = useState(ad.pickupCoords || null);
  const [modalVisible, setModalVisible] = useState(false);
  const [region, setRegion] = useState(
    ad.pickupCoords
      ? {
          latitude: ad.pickupCoords.latitude,
          longitude: ad.pickupCoords.longitude,
          latitudeDelta: 0.01,
          longitudeDelta: 0.01,
        }
      : null
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      const imageUrls = await uploadAllImages(userId, ad.id);
