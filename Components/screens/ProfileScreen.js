...
export default function ProfileScreen() {
  ...
  const updatePreference = async (key, val) => {
    ...
    if (!uid) return;
    try {
      await updateDoc(doc(db, 'users', uid, 'preferences', 'general'), { [key]: val });
    } catch (e) {
        ...
        <TouchableOpacity style={styles.card}>
          <View style={styles.cardLeft}>
            ...
            <View style={{ marginLeft: 14, flex: 1 }}>
              ...
              <Text style={styles.cardSubtitle}>For donation every day without forget</Text>
            </View>
          </View>
...
const base = {
  ...
  newTag: { paddingHorizontal: 10, paddingVertical: 2, borderRadius: 12, marginLeft: 8 },
};
...
const lightStyles = StyleSheet.create({
  ...
  toggleText: { fontSize: 16, marginLeft: 16, color: '#000', fontWeight: '500' },
...
const darkStyles = StyleSheet.create({
  ...
  toggleText: { fontSize: 16, marginLeft: 16, color: '#fff', fontWeight: '500' },
